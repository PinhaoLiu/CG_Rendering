#include "simulation/CpuSphSolver.hpp"
...
namespace sph
{
	CpuSphSolver::CpuSphSolver(std::size_t const maximum_particle_count,
	...
	std::size_t CpuSphSolver::spawnParticles(std::vector<ParticleSpawn> const& particles)
	{
	...
	void CpuSphSolver::setBoundary(BoxBoundary const& boundary)
	{
		mBoundary = boundary;
	...
	void CpuSphSolver::step(float const fixed_delta_seconds)
	{
	...
	std::vector<glm::vec3> const& CpuSphSolver::positions() const noexcept
	{
		return mRenderPositions;
	}

	std::size_t CpuSphSolver::capacity() const noexcept
	{
		return mMaximumParticleCount;
	}

	char const* CpuSphSolver::backendName() const noexcept
	{
		return "CPU WCSPH AoS O(N^2) reference";
	}

	float CpuSphSolver::particleRadius() const noexcept
	{
		return mParameters.particleRadius;
	}

	SphParameters const& CpuSphSolver::parameters() const noexcept
	{
		return mParameters;
	}

	void CpuSphSolver::setParameters(SphParameters const& parameters)
	{
	...
	SphDiagnostics const& CpuSphSolver::diagnostics() const noexcept
	{
		return mDiagnostics;
	}

	void CpuSphSolver::validateParameters(SphParameters const& parameters)
	{
	...
	void CpuSphSolver::computeDensityAndPressure()
	{
		mDiagnostics.minimumDensity = std::numeric_limits<float>::max();
		mDiagnostics.maximumDensity = 0.0f;
		double density_sum = 0.0;

		for (Particle& particle_i : mParticles) {
			float density = 0.0f;
			for (Particle const& particle_j : mParticles) {
	...
	void CpuSphSolver::computeAcceleration()
	{
		...
		for (std::size_t i = 0u; i < mParticles.size(); ++i) {
			...
			for (std::size_t j = 0u; j < mParticles.size(); ++j) {
				if (i == j)
					continue;
	...
	void CpuSphSolver::integrate(float const fixed_delta_seconds)
	{
	...
	void CpuSphSolver::collideWithBoundary(Particle& particle) const
	{
	...
	void CpuSphSolver::rebuildRenderPositionsAndDiagnostics()
	{
